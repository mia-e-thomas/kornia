...
class RANSAC(Module):
    ...
    def verify(self, kp1: Tensor, kp2: Tensor, weights: Tensor, models: Tensor, inl_th: float) -> Tuple[Tensor, Tensor, float]:
        ...
        inl = errors <= inl_th # Boolean, BxN, no grad
        # Score
        models_score = (inl*weights).to(kp1).sum(dim=1) # B, contains grad (because of weights)
        # Best
        best_model_idx = models_score.argmax() # int, no grad
        best_model_score = models_score[best_model_idx].item() # float, no grad
        model_best = models[best_model_idx].clone() # 3x3, grad
        inliers_best = inl[best_model_idx] # N (bool), no grad
        return model_best, inliers_best, best_model_score

    def remove_bad_samples(self, kp1: Tensor, kp2: Tensor, weights: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        """"""
    ...
    def forward(self, kp1: Tensor, kp2: Tensor, weights: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        ...
        Args:
            ...
            weights: correspondences weights :math:`(N)`.
